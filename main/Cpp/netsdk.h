...
typedef enum _SDK_CONFIG_TYPE
{
	...
	E_SDK_CFG_PLATEDETECT,					//车牌侦测--SDK_PlateDetectConfig
...
typedef struct SDK_Net3GConfig
{
    ...
    CONFIG_IPAddress addr;			  		///< 拨号后获得的IP地址
	char sOperators[NET_NAME_PASSWORD_LEN]; ///< 运营商
}SDK_Net3GConfig;
...
//车牌侦测
typedef struct SDK_PlateDetectConfig
{
	bool nEnable;					//使能开关
	int  iDefaultL;					//默认字母,比如"A"
	int  iDefaultP;					//默认汉字,比如"浙"
	int  uiPlateType;				//特征位置--见enum SDK_PlateType结构体
	SDK_PlateRect stPlateRect[7];	//车牌区域
	char vWhiteListAddr[6][256];	//车牌白名单地址--暂不使用
	int iMaxWidth;					//最小车牌检查宽度--以像素为单位-默认为100 范围50-250
	int iMinWidth;					//最大车牌检查宽度--以像素为单位-默认为160 范围50-250
}SDK_PlateDetectConfig;
//所有通道的车牌侦测
typedef struct SDK_PlateDetectConfigAll
{
	SDK_PlateDetectConfig vPlateDetectConfigAll[NET_MAX_CHANNUM];
}SDK_PlateDetectConfigAll;
...
*/
typedef bool (CALL_METHOD *fMessCallBack)(long lLoginID, char *pBuf, unsigned long dwBufLen, long dwUser);
